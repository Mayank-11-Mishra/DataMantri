...
const DashboardRenderer: React.FC<DashboardRendererProps> = ({ spec, onSave, editable = false }) => {
  ...
  // Query and chart type editing states
  const [editingChartId, setEditingChartId] = useState<string | null>(null);
  const [editedQuery, setEditedQuery] = useState<string>('');
  const [editedChartType, setEditedChartType] = useState<string>('');
  ...
  const fetchChartData = async (chart: ChartConfig) => {
    ...
    try {
      // AI Insights charts have pre-computed data, no query needed
      if (chart.type === 'ai-insights') {
        console.log(`[DashboardRenderer] AI Insights chart ${chart.id} - using pre-computed data`);
        // The data is already in chart.data
        const aiData = (chart as any).data || { keyFindings: [], trends: [], anomalies: [], recommendations: [], summary: '' };
        setChartData(prev => ({ ...prev, [chart.id]: aiData }));
      ...
      // Call run-query API - Check for dataSourceId in multiple locations
      const dataSourceId = (chart as any).dataSourceId || spec.dataSourceId || 'default';
  ...
  const handleSaveQuery = async () => {
    ...
    try {
      ...
      // Update the chart query AND type
      spec.charts = spec.charts.map(c => 
        c.id === editingChartId ? { ...c, query: editedQuery, type: editedChartType } : c
      );
  ...
  return (
    ...
    >
      ...
      {/* Charts Grid - User's Requested Layout Order */}
      <div className="space-y-6" style={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
        
        {/* 1. KPI Cards Row - Top (4 columns) */}
        {spec.charts.filter(c => c.type === 'kpi').length > 0 && (
          <div className={`grid gap-6 ${
            ...
            {spec.charts.filter(c => c.type === 'kpi').map(chart => (
              <div key={chart.id} className="relative bg-white rounded-xl shadow-lg overflow-hidden group">
                <div className="absolute top-2 right-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                  <button
                    onClick={() => handleEditQuery(chart.id)}
                    className="p-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 shadow-lg transition flex items-center gap-1 text-xs font-semibold"
                    title="Edit Chart"
                  >
        ...
        {/* 2. Pie and Bar Charts - 2 columns */}
        {spec.charts.filter(c => c.type === 'pie' || c.type === 'bar').length > 0 && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {spec.charts.filter(c => c.type === 'pie' || c.type === 'bar').map(chart => (
              <div key={chart.id} className="relative bg-white rounded-xl shadow-lg overflow-hidden min-w-0 group">
                <div className="absolute top-2 right-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                  <button
                    onClick={() => handleEditQuery(chart.id)}
                    className="p-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 shadow-lg transition flex items-center gap-1 text-xs font-semibold"
        ...
        {/* 3. Line/Area Charts - 2 columns or full width if only 1 */}
        {spec.charts.filter(c => c.type === 'line' || c.type === 'area').length > 0 && (
          <div className={`grid gap-6 ${
            spec.charts.filter(c => c.type === 'line' || c.type === 'area').length === 1 ? 'grid-cols-1' : 'grid-cols-1 lg:grid-cols-2'
          }`}>
            {spec.charts.filter(c => c.type === 'line' || c.type === 'area').map(chart => (
              <div key={chart.id} className="relative bg-white rounded-xl shadow-lg overflow-hidden group">
                <div className="absolute top-2 right-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                  <button
                    onClick={() => handleEditQuery(chart.id)}
        ...
        {spec.charts.filter(c => c.type === 'table').map(chart => (
          <div key={chart.id} className="relative bg-white rounded-xl shadow-lg overflow-hidden min-w-0 w-full group">
            <div className="absolute top-2 right-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
              <button
                onClick={() => handleEditQuery(chart.id)}
                className="p-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 shadow-lg transition flex items-center gap-1 text-xs font-semibold"
        ...
        {/* 5. AI Insights - Full width at BOTTOM */}
        {spec.charts.filter(c => c.type === 'ai-insights').map(chart => (
          <div key={chart.id} className="relative bg-white rounded-xl shadow-lg overflow-hidden min-w-0 w-full">
            {renderChart(chart)}
          </div>
        ))}

        {/* 6. Other chart types - 2 columns */}
        {spec.charts.filter(c => !['kpi', 'line', 'area', 'bar', 'pie', 'table', 'ai-insights'].includes(c.type)).length > 0 && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {spec.charts.filter(c => !['kpi', 'line', 'area', 'bar', 'pie', 'table', 'ai-insights'].includes(c.type)).map(chart => (
              <div key={chart.id} className="relative bg-white rounded-xl shadow-lg overflow-hidden group">
                <div className="absolute top-2 right-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                  <button
                    onClick={() => handleEditQuery(chart.id)}
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="bg-gradient-to-r from-blue-600 to-indigo-700 p-6 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-xl font-bold text-white">Edit Chart</h3>
            ...
            <div className="flex-1 p-6 overflow-y-auto">
              ...
              <div className="mb-4">
                ...
                <select
                  value={editedChartType}
                  onChange={(e) => setEditedChartType(e.target.value)}
                  className="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:border-blue-500 focus:outline-none bg-white text-gray-900 font-medium"
                >
              ...
              <div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-4 mb-4">
                ...
                <ul className="text-sm text-blue-800 space-y-1">
                  <li>• Change chart type above to switch between Bar, Line, Pie, etc.</li>
