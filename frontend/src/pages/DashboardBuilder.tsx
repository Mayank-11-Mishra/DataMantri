  ...
  const loadDashboardForEdit = async (dashboardId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        // Check if dashboard was created with AI (has ai_generated flag or specific structure)
    ...
    return (
      <div className="h-screen flex flex-col bg-gradient-to-br from-gray-50 to-blue-50">
        <div className="p-6 bg-white border-b shadow-sm flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-gray-900">Visual Dashboard Builder</h2>
    ...
    return (
      <div className="h-screen flex flex-col bg-gradient-to-br from-gray-50 to-blue-50">
        <div className="p-6 bg-white border-b shadow-sm flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-gray-900">AI Dashboard Builder</h2>
  ...
  return (
    <div className="w-full min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 p-8">
      ...
      <div>
              ...
              <p className="text-gray-600 text-[15px] mt-1">Create stunning dashboards with AI or visual tools</p>
            </div>
          </div>
      </div>
      ...
      {/* Create New Dashboard Section */}
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden">
          <div className="p-6 bg-gradient-to-r from-indigo-50 to-purple-50 border-b-2 border-indigo-100">
            ...
            <p className="text-gray-600 text-[14px] mt-2">Choose your preferred method to build your next great dashboard</p>
          </div>

          <div className="p-6">
          {/* Creation Type Selection */}
            <div className="mb-6">
              <h3 className="text-base font-bold text-gray-900 mb-5">Select Creation Method</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {creationOptions.map((option) => (
                  <button
                  key={option.id}
                    onClick={() => setSelectedCreationType(option.id)}
                    className={`text-left p-6 rounded-xl border-2 transition-all duration-300 group ${
                      selectedCreationType === option.id
                        ? `border-${option.gradient.split(' ')[0].replace('from-', '')} bg-gradient-to-br ${option.bgGradient} shadow-xl scale-[1.02]`
                        : 'border-gray-200 bg-white hover:border-indigo-300 hover:shadow-lg hover:scale-[1.01]'
                    }`}
                  >
                    <div className="flex items-start gap-4 mb-3">
                      <div className={`w-14 h-14 bg-gradient-to-br ${option.gradient} rounded-xl flex items-center justify-center shadow-md ${
                        selectedCreationType === option.id ? 'scale-105' : 'group-hover:scale-105'
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {selectedCreationType === option.id && (
                            <span className="px-2 py-0.5 bg-gradient-to-r from-green-500 to-emerald-600 text-white text-[10px] font-bold rounded-full">
                              ✓ SELECTED
                            </span>
                          )}
                        </div>
                        <p className="text-gray-600 text-[13px] leading-relaxed">{option.description}</p>
                      </div>
                    </div>
            ...
            {/* Direct Builder Launch - No name needed here */}
          {selectedCreationType && (
              <div className="space-y-5 border-t-2 border-gray-200 pt-6 animate-fade-in">
                <div className="flex justify-center">
                  <button
                    onClick={() => {
                      // Open builder directly (no name required, will be entered in builder)
                      setIsBuilderOpen(true);
                    }}
                    className={`px-10 py-4 rounded-xl font-semibold text-base transition-all duration-300 flex items-center gap-3 ${
                      selectedCreationType === 'visual'
                        ? 'bg-gradient-to-r from-purple-500 to-pink-600 hover:from-purple-600 hover:to-pink-700 text-white shadow-lg hover:shadow-xl hover:scale-[1.02]'
                        : 'bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white shadow-lg hover:shadow-xl hover:scale-[1.02]'
                    }`}
                  >
                    {selectedCreationType === 'visual' ? (
                      <>
                ...
                <p className="text-center text-gray-600 text-sm">
                  {selectedCreationType === 'visual' 
                    ? 'Configure your dashboard name and settings inside the builder'
                    : 'Enter your prompt and dashboard details inside the builder'
                  }
                </p>
              </div>
            )}
